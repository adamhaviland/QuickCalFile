...
(function () {
  ...
  function parseDateOnly(value) {
    ...
    const dmy = raw.replace(/\s+/g, ''); const dmyMatch = dmy.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/); if (dmyMatch) { const dd = Number(dmyMatch[1]); const mm = Number(dmyMatch[2]); const yy = Number(dmyMatch[3]); return new Date(yy, mm - 1, dd); }
  ...
  function validate() {
    const rows = bulkList ? Array.from(bulkList.querySelectorAll('.bulk-row')) : []; if (rows.length === 0) return 'Add at least one row.';
    for (const row of rows) {
      ...
      const isTimed = row.classList.contains('is-times');
      if (!title || !date) return 'Each row needs a title and date.';
      if (isTimed) {
        if (!start || !end) return 'Provide start and end times or select All day.';
        const [sh, sm] = start.split(':').map(Number); const [eh, em] = end.split(':').map(Number);
        if (eh * 60 + em <= sh * 60 + sm) return 'Row end time must be after start time.';
      }
    }
    return '';
  }
  ...
  function generateICS() {
    const now = new Date(); const dtStampUtc = new Date(now.getTime() - now.getTimezoneOffset() * 60000).toISOString().replace(/[-:]/g, '').replace(/\..+/, 'Z');
    const calLines = [ 'BEGIN:VCALENDAR', 'PRODID:-//Quick Cal File//EN', 'VERSION:2.0', 'CALSCALE:GREGORIAN', 'METHOD:PUBLISH', 'X-WR-CALNAME:Quick Cal File', 'X-WR-CALDESC:Generated with Quick Cal File' ];
    const baseUid = Math.random().toString(36).slice(2); const rows = bulkList ? Array.from(bulkList.querySelectorAll('.bulk-row')) : [];
    rows.forEach((row, i) => {
      const titleRaw = (row.querySelector('.bulk-title') || {}).value || ''; const dateVal = (row.querySelector('.bulk-date') || {}).value || '';
      const startVal = (row.querySelector('.bulk-start') || {}).value || ''; const endVal = (row.querySelector('.bulk-end') || {}).value || ''; const locRaw = (row.querySelector('.bulk-loc') || {}).value || '';
      const isTimed = row.classList.contains('is-times'); const date = parseDateOnly(dateVal); if (!date || isNaN(date.getTime())) return;
      const dateBasic = formatDateToBasic(date); let dtStartLine = ''; let dtEndLine = '';
      if (!isTimed || (!startVal && !endVal)) { const endAllDay = new Date(date); endAllDay.setDate(endAllDay.getDate() + 1); dtStartLine = `DTSTART;VALUE=DATE:${dateBasic}`; dtEndLine = `DTEND;VALUE=DATE:${formatDateToBasic(endAllDay)}`; }
      else { const [lsh, lsm] = (startVal || '00:00').split(':').map(Number); const [leh, lem] = (endVal || '00:00').split(':').map(Number); const dtStart = `${dateBasic}T${formatTimeToBasic(lsh, lsm)}`; const dtEnd = `${dateBasic}T${formatTimeToBasic(leh, lem)}`; dtStartLine = `DTSTART:${dtStart}`; dtEndLine = `DTEND:${dtEnd}`; }
      const uid = `${dateBasic}-${i}-${baseUid}@quickcalfile.local`; const title = escapeICSText(titleRaw.trim() || 'Event'); const loc = escapeICSText(locRaw.trim());
      calLines.push('BEGIN:VEVENT'); calLines.push(`UID:${uid}`); calLines.push(`DTSTAMP:${dtStampUtc}`); calLines.push(dtStartLine); calLines.push(dtEndLine); calLines.push(`SUMMARY:${title}`); if (loc) calLines.push(`LOCATION:${loc}`); calLines.push('END:VEVENT');
    });
  ...
  function updatePill(pill, row) {
    const display = row.querySelector('.time-display');
    if (!row.classList.contains('is-times')) {
      pill.textContent = 'All day';
      if (display) display.textContent = '';
      return;
    }
    const s = (row.querySelector('.bulk-start') || {}).value || '';
    const e = (row.querySelector('.bulk-end') || {}).value || '';
    const text = s && e ? `${formatTimeHuman(s)} – ${formatTimeHuman(e)}` : 'Time';
    pill.textContent = text;
    if (display) display.textContent = s && e ? text : '';
  }

  function openTimePopover(pill, row) {
    closeTimePopover();
    const overlay = document.createElement('div'); overlay.className = 'time-popover-overlay';
    ...
    pop.style.left = `${Math.min(rect.left + window.scrollX, window.innerWidth - 320)}px`;

    const allDay = pop.querySelector('.pp-all-day');
    const start = pop.querySelector('.pp-start');
    const end = pop.querySelector('.pp-end');
    const applyBtn = pop.querySelector('.pp-apply');

    const startVal = (row.querySelector('.bulk-start') || {}).value || '';
    const endVal = (row.querySelector('.bulk-end') || {}).value || '';
    if (startVal) start.value = startVal; if (endVal) end.value = endVal;

    function syncState() {
      const timed = !allDay.checked;
      start.disabled = !timed; end.disabled = !timed;
      if (!timed) { applyBtn.disabled = false; return; }
      const ok = !!start.value && !!end.value && (end.value > start.value);
      applyBtn.disabled = !ok;
    }

    function apply() {
      if (allDay.checked) { switchToAllDay(row); updatePill(pill, row); closeTimePopover(); return; }
      switchToTimes(row);
      const s = row.querySelector('.bulk-start'); const e = row.querySelector('.bulk-end');
      s.value = start.value; e.value = end.value;
      updatePill(pill, row); closeTimePopover();
    }

    pop.querySelector('.pp-apply').addEventListener('click', apply);
    pop.querySelector('.pp-cancel').addEventListener('click', () => closeTimePopover());
    // Do NOT close on overlay click to avoid accidental dismiss
    // overlay.addEventListener('click', () => closeTimePopover());

    allDay.addEventListener('change', syncState);
    start.addEventListener('input', syncState);
    end.addEventListener('input', syncState);
    syncState();
  }

  function switchToTimes(row) {
    row.classList.add('is-times');
    const cell = row.querySelector('.time-group');
    cell.innerHTML = `
      <div>
    ...
    `;
    const pill = cell.querySelector('.time-pill');
    pill.addEventListener('click', () => openTimePopover(pill, row));
    updatePill(pill, row);
  }

  function switchToAllDay(row) {
    row.classList.remove('is-times');
    const cell = row.querySelector('.time-group');
    cell.innerHTML = `<div><button type="button" class="time-pill" aria-haspopup="dialog">All day</button><div class="time-display"></div></div><input class="bulk-start" type="time" style="display:none" /><input class="bulk-end" type="time" style="display:none" />`;
    const pill = cell.querySelector('.time-pill');
    pill.addEventListener('click', () => openTimePopover(pill, row));
  }
  ...
  function createBulkRow() {
    ...
    `;
    row.querySelector('.remove').addEventListener('click', () => { row.remove(); refreshRecap(); });
    ['input','change'].forEach(evt => { row.querySelectorAll('input').forEach(el => el.addEventListener(evt, refreshRecap)); });
    switchToAllDay(row);
    return row;
  }
  ...
  (function initDefaults() { refreshRecap(); initTheme(); })();
})();
